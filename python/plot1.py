...
for i in range(1000,1250):
    ...
    plt.text(pulse_time+2.5, np.min(order_param_pulsed[1]), pulse_time, fontsize=17)
    plt.ylim(0, 1)
    ...
    plt.subplot(2, 1, 2)
    plt.plot(order_param[0][:],order_param[2][:],'r',label='phases')
    plt.plot(order_param[0][:],np.sin(order_param[2][:]),'black',label='sin phases')
    plt.vlines(pulse_time,np.min(order_param_pulsed[1]),np.max(order_param_pulsed[1]))
    plt.legend(loc='upper right')
